...
/** Adds passes that Three auto-reset out of live counters without double counting. */
export function drawStatWithManualPasses(
  baseCount: number,
  passCount: number,
  primitivesPerPass: number,
  alreadyIncluded: boolean,
): number {
  if (alreadyIncluded) return baseCount;
  return baseCount + Math.max(0, passCount) * Math.max(0, primitivesPerPass);
}
