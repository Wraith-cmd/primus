...
// ---------------------------------------------------------------------------
// The public entry point an out-of-band footer write uses. refreshIfChanged is the
// purse-driven probe and elides on an unmoved purse; refreshMoneyRow is the
// unconditional repaint. Driving it directly is what keeps those two from
// collapsing into one.
// ---------------------------------------------------------------------------

describe('BagsWindow.refreshMoneyRow (the unconditional repaint entry point)', () => {
  it('repaints even when the purse has NOT moved', () => {
    // The whole reason it is a separate public method: a refreshMoneyRow that
    // consulted the lastMoneyCopper latch (or delegated to refreshIfChanged) would
    // silently never repaint an unmoved purse.
    const h = harness(1000);
    h.window.render();
    expect(h.paints()).toBe(1);
