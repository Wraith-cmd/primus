...
const mainSide = read('electron/main.cjs');
...
describe('electron IPC channel contract (preload <-> main)', () => {
  ...
  it('every preload subscription has a main-side webContents.send', () => {
    ...
    expect([...subscribed].sort()).toEqual(['desktop-login-code', 'desktop-wallet-handoff-code']);
  ...
  it('every ipcMain.handle body checks the trusted-sender gate FIRST', () => {
    ...
    // main.cjs handlers all call trustedSender(...); the check must appear
    // within the first statement's reach of the callback body.
