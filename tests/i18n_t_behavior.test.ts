...
describe('t(): pending key (untranslated; the dense table English-fills it)', () => {
  ...
  // A pending key in the ENGLISH-ONLY dev namespace. Same shape as SAMPLE, but the
  // release hard-fail must NOT fire for it: see isEnglishOnlyNamespace in i18n.ts.
  const DEV_SAMPLE = 'devCommand.__samplePendingKey';
  ...
  async function loadWithPending() {
    ...
    vi.doMock(ES, async () => {
      ...
      // t() splits a dotted key and WALKS the table, so the dev fixture has to be
      // a real nested branch, not a flat "devCommand.x" string key.
      const table = {
        ...actual.es,
        [SAMPLE]: FILL,
        devCommand: {
          ...((actual.es as Record<string, unknown>).devCommand as Record<string, string>),
          __samplePendingKey: FILL,
        },
      };
    ...
    vi.doMock(PENDING, async () => {
      ...
      return {
        pending: { ...actual.pending, es: [...(actual.pending.es ?? []), SAMPLE, DEV_SAMPLE] },
      };
    });
  ...
  // REGRESSION: the /dev command GUI shipped nine English-only keys, which land
  // pending in all 19 non-en locales. Before the exemption this threw on a release
  // build, and because the throw happened mid-template the window's markup was never
  // assigned AND its category had already mutated, so every later render threw too:
  // the panel was dead until reload. English on a cheat panel beats a bricked panel.
  it('does NOT hard-fail for the English-only dev namespace on a release build', async () => {
    process.env.I18N_RELEASE = '1';
    const mod = await loadWithPending();
    mod.setLanguage('es');
    await mod.ensureLocaleLoaded('es');
    const tm = mod.t as unknown as (k: string, v?: Record<string, string | number>) => string;
    expect(() => tm('devCommand.__samplePendingKey', { name: 'Aki' })).not.toThrow();
    expect(tm('devCommand.__samplePendingKey', { name: 'Aki' })).toBe('English fill Aki');
  });

  // Teeth: the exemption must be a NAMESPACE rule, not a blanket softening of the
  // release gate. A player-facing key that merely CONTAINS the word stays fatal.
  it('keeps the hard-fail for every namespace that ships to players', async () => {
    const mod = await import('../src/ui/i18n');
    expect(mod.isEnglishOnlyNamespace('devCommand.actions.vendor.label')).toBe(true);
    expect(mod.isEnglishOnlyNamespace('hudChrome.devCommand.label')).toBe(false);
    expect(mod.isEnglishOnlyNamespace('mode.runCta')).toBe(false);
  });
});
