...
// The /internal/ arm dispatches inside an async IIFE, so drain the microtask
// queue before asserting the fire-and-forget dispatch landed.
...
async function loadRoute() {
  ...
  // fire on the legacy delegate path. The production boot default is 'new', where
  // a MATCHED migrated path (/internal/restart-countdown) runs the onion instead
